...
function Register({ setIsLoggedIn }: RegisterProps) {
    ...
    const {
        ...
        watch,
        formState: { errors }
    } = useForm<RegisterProfile>();


    let onSubmit = async (data: RegisterProfile) => {
        console.log(data);

        try {
            setErrorMsg('');
            setSuccesMsg('');
            let { cpassword, ...rest } = data
            const { data: registeredUser } = await axios.post(`${BASE_API_URL}/users`, rest)
